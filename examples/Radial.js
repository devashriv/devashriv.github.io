class Radial {
  constructor({
    ...
    if (isNaN(this.fontSize)) throw "Font size must be a number";
    ...
    if (this.innerCircle < 0) throw "Inner circle can't be negative";
  ...
  init() {
    ...
    this.w =
      this.outerCircle * 2 + this.shadowBlur * 2 + this.shadowOffsetX * 2;

    this.h =
      this.outerCircle * 2 + this.shadowBlur * 2 + this.shadowOffsetY * 2;
    ...
    for (let i = 0; i < this.buttons.length; i++) {
      ...
      this.buttons[i]["fin"] =
        (this.buttons[i]["ini"] + this.step) % this.TWOPI;

      if (this.buttons[i]["ini"] > this.buttons[i]["fin"]) this.rest = i;

      const a = this.buttons[i]["ini"] + this.step / 2;

      this.buttons[i]["centerX"] =
        Math.cos(a) *
          (this.innerCircle + (this.outerCircle - this.innerCircle) / 2) -
        this.fontSize / 2;

      this.buttons[i]["centerY"] =
        Math.sin(a) *
          (this.innerCircle + (this.outerCircle - this.innerCircle) / 2) +
        this.fontSize / 4;
    }
  ...
  draw() {
    ...
    for (let i = 0; i < this.buttons.length; i++) {
      ...
      this.c.fillStyle =
        "backgroundColor" in button
          ? button["backgroundColor"]
          : this.backgroundColor;

      this.c.strokeStyle =
        "borderColor" in button ? button["borderColor"] : this.borderColor;
      ...
      this.c.fillStyle =
        "textColor" in button ? button["textColor"] : this.textColor;

      this.c.strokeStyle =
        "textBorderColor" in button
          ? button["textBorderColor"]
          : this.textBorderColor;

      this.c.shadowColor =
        "textShadowColor" in button
          ? button["textShadowColor"]
          : this.textShadowColor;

      this.c.shadowBlur =
        "textShadowBlur" in button
          ? button["textShadowBlur"]
          : this.textShadowBlur;

      this.c.shadowOffsetX =
        "textShadowOffsetX" in button
          ? button["textShadowOffsetX"]
          : this.textShadowOffsetX;

      this.c.shadowOffsetY =
        "textShadowOffsetY" in button
          ? button["textShadowOffsetY"]
    ...
    if (iOS) {
      document.addEventListener("touchstart", (e) => {
        this.timer = setTimeout(() => {
          this.setPos(
            e.touches[0].clientX - this.w2,
            e.touches[0].clientY - this.h2
          );
  ...
  setPos(x, y) {
    if (isNaN(x) || isNaN(y)) throw "X and Y must be numbers";
  ...
  createGradient(obj) {
    if (!("gradient" in obj)) throw "Invalid gradient object";

    let gradient;

    switch (obj["gradient"]) {
      case "radial":
        gradient = this.c.createRadialGradient(
          this.w2,
          this.h2,
          this.innerCircle,
          this.w2,
      ...
      case "linear3":
        gradient = this.c.createLinearGradient(
          0,
          0,
          this.canvas.width,
          this.canvas.height
    ...
    for (const key in obj["colors"]) {
      if (isNaN(key) || key < 0 || key > 1) throw "Invalid color position";
